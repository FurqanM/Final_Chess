...
public class Queen extends Piece
{
	...
	public boolean validateMovement(String position)
	{
		...
		/*
		 * String undatedPosition = (position.substring(3, 4).trim().toLowerCase() + position.substring(4).trim()); //c5 char[] charSequence = {(char) ((newRank + 1) + 97) , (char) ((newFile + 1) +
		 * 49)}; // d6 String charString = String.copyValueOf(charSequence); // d6 String updatedPosition = undatedPosition + " " + charString;
		 */
		/*
		 * int greaterSquare = 0; int smallerSquare = 0; if(initialFile < newFile) { greaterSquare = newFile; smallerSquare = initialFile; } else { greaterSquare = initialFile; smallerSquare =
		 * newFile; }
		 */
